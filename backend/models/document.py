...
class MasterDocument(Base):
    ...
    original_file_path = Column(NVARCHAR(500), nullable=True)
    ...
    document_type = Column(NVARCHAR(100), nullable=True)
    project_id = Column(NVARCHAR(100), ForeignKey("project.project_id", ondelete="CASCADE"), nullable=True, index=True)
    ...
    confidence_scores = Column(NVARCHAR("MAX"), nullable=True)
